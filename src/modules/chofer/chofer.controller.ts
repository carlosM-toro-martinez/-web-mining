...
import { choferQuerySchema } from "./chofer.schema.js";
...
export const choferController = {
  async getAll(req: AuthRequest, res: Response) {
    try {
      ...
      const query = choferQuerySchema.parse(req.query);
      const data = await choferService.getAll(query);
  ...
  async getById(req: AuthRequest, res: Response) {
    try {
      ...
      res.json({ success: true, data: chofer });
    } catch (error) {
      const status = error instanceof HttpError ? error.statusCode : 500;
      res.status(status).json({ success: false, error: (error as Error).message });
    }
  ...
  async create(req: AuthRequest, res: Response) {
    try {
      const chofer = await choferService.create(req.body, req.user!.id);
  ...
  async update(req: AuthRequest, res: Response) {
    try {
      const id = Number(req.params.id);
      const chofer = await choferService.update(id, req.body, req.user!.id);
      res.json({ success: true, data: chofer });
    } catch (error) {
      const status = error instanceof HttpError ? error.statusCode : 400;
      res.status(status).json({ success: false, error: (error as Error).message });
